...
const HotelCard: FunctionComponent<HotelCardProps> = ({hotel, destination}) => {
    return (
        <div key={hotel.id} className="overflow-hidden rounded-md shadow-md flex flex-col">
            ...
            <div className="grow border border-black/10 rounded-b-md bg-white dark:bg-gray-900 py-3 px-2 flex flex-col items-center justify-between gap-2 text-sm">
                ...
                <div className="w-full flex items-center justify-center relative pt-2 pb-3">
                    <hr className="w-full"/>
                    <span className="absolute text-xs flex flex-row-reverse justify-center gap-1 bg-zinc-600 dark:bg-gray-950 rounded-full py-1 px-2 w-fit">
                        ...
                        {Array.from({length: 5 - hotel.stars}).map((x, i) => (
                            <i key={i} className="h-3 fi fi-rs-star text-gray-200 dark:text-gray-600"></i>
                        ))}
                    </span>
                </div>
                <span className="flex gap-2 text-gray-500 dark:text-gray-400">
