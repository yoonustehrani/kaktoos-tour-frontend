...
const staticClassNames = 'rounded-md p-3 border-2 flex flex-col-reverse md:flex-row gap-3 items-center justify-center text-sm md:text-base'

function Div(props: PropsWithChildren) {
    return <div className={`${staticClassNames} text-blue-500 dark:border-darkBlue-marian-light`}>{props.children}</div>
}
...
function DatePill(
    ...
    return (
        <Comp href={`?date=${date.id}`} className={`${staticClassNames} text-gray-400 dark:border-gray-500 hover:dark:border-darkBlue-marian-light`}>                 
